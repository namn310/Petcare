...
function checkPasswordSigup(){
    var correct_password=/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])([a-zA-Z0-9]{8})$/;
...
function checknameBookingForm(){
    var name_correct=/^[A-Za-z\sAÀẢÃÁẠĂẰẲẴẮẶÂẦẨẪẤẬBCDĐEÈẺẼÉẸÊỀỂỄẾỆFGHIÌỈĨÍỊJKLMNOÒỎÕÓỌÔỒỔỖỐỘƠỜỞỠỚỢPQRSTUÙỦŨÚỤƯỪỬỮỨỰVWXYỲỶỸÝỴZaàảãáạăằẳẵắặâầẩẫấậbcdđeèẻẽéẹêềểễếệfghiìỉĩíịjklmnoòỏõóọôồổỗốộơờởỡớợpqrstuùủũúụưừửữứựvwxyỳỷỹýỵz]+$/;
...
if(Bossweight_val=="" || weight_correct.test(Bossweight_val)==false){
...
function completeBooking(){
    if(checknameBookingForm()==true && checktypeBookingForm()==true && checkweightBookingForm()==true){
        alert("Đặt lịch thành công! Bạn có thể kiểm tra lại thông tin trong giỏ hàng.")
    }
}
...
function increase_count(){
    var current_count_product=document.getElementById("current_count").value;
    document.getElementById("current_count").value = increase(parseInt(current_count_product));
...
function decrease_count(){
    ...
    document.getElementById("current_count").value = decrease(parseInt(current_count_product));
    var giatien=document.getElementById("giatien").innerHTML;
...
function checkCustomerDetailPhone(){
    var correct_phone=/^(0?)(3[2-9]|5[6|8|9]|7[0|6-9]|8[0-6|8|9]|9[0-4|6-9])[0-9]{7}$/;
...
function verify(){
    if(checkCustomerDetailName()==true && checkCustomerDetailPhone()==true && checkCustomerDetailLocate()==true && checkCustomerDetailEmail() == true && checkCity()==true && checkDistrict()==true && checkWard()==true){
...
function sendContactForm(){
    ...
    if(checkCustomerDetailName()==true && checkCustomerDetailPhone()==true  && checkCustomerDetailEmail() == true && message!="" ){
        $("#error_message").hide();
       alert("Cảm ơn đã đóng góp ý kiến ! Chúng tôi sẽ sớm liên hệ với bạn")
    }
